...
class Run(object):
    ...
    def generate_id(self, parameters):
        ...
        id_dict = {key: self._serialize(val) for key, val in parameters.items() if
                   not self._serialize(val) == '__unhashable__'}
        ...
        if isinstance(val, types.CodeType):
            ...
            if serialized == '__unhashable__':
                ...
                if len(string) > 0 and string[0] == '<' and string[-1] == '>' and 'at 0x' in string:
                    print(
                        'WARNING: parameter {} can not be hashed and is not included in the id'
                        + 'which could lead to loss of data and undesired results'.format(val)
                    )
                else:
                    serialized = val
